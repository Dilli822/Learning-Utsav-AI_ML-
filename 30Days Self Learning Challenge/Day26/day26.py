...
url = "https://archive.ics.uci.edu/ml/machine-learning-databases/iris/iris.data"  # Change this to any dataset URL
...
num_features = df.shape[1]  # Total number of columns in the dataset
df.columns = [f'Feature_{i+1}' for i in range(num_features)]  # Generic column names
...
def animate_feature_selection():
    global current_step
    num_features = df.shape[1]  # Total number of features
    
    while animation_running and current_step < num_features:
        features_to_test = df.iloc[:, :current_step + 1]  # Include features up to current_step
        ...
        ax.clear()
        for feature in df.columns:  # Loop through all features
